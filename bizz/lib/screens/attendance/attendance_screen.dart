...
import '../../widgets/app_action_icon.dart';
...
class _AttendanceScreenState extends State<AttendanceScreen>
  ...
  */
  ...
  Widget _buildCustomDay(DateTime day) {
    ...
    final bool isLeaveFromBackend = leaveDates.contains(dateStr);
    ...
    } else if (statusFromRecord == 'On Leave' || isLeaveFromBackend) {
    ...
    return RefreshIndicator(
      ...
      child: SingleChildScrollView(
        ...
        child: Column(
          ...
          children: [
            Text(
              'TODAY - ${DateFormat('MMM dd, yyyy').format(_focusedDay).toUpperCase()}',
              style: TextStyle(
                fontSize: 12,
                fontWeight: FontWeight.w600,
            ...
            const SizedBox(height: 12),
            _buildCheckInHistorySection(limit: 5),
          ],
        ),
      ),
    );
  ...
  Widget _buildCheckInHistorySection({int? limit}) {
    final colors = AppColors.of(context);
    ...
    final displayList = _getCombinedMonthHistory();
    final list = limit != null ? displayList.take(limit).toList() : displayList;
    if (list.isEmpty) {
      return Center(
        child: Padding(
          padding: const EdgeInsets.all(32),
    ...
    return _buildHistoryList(limit: limit);
  }
  ...
  Widget _buildStatusLegend() {
    return Wrap(
      ...
      children: [
        ...
        _legendItem(Colors.blue.shade100, 'On Leave'),
  ...
  Widget _buildAttendanceCard() {
    ...
    const punchInBg = Color(0xFF2F5C5B);
    const punchOutBg = Color(0xFF2B3049);
    const punchOutIconBg = Color(0xFFE0505F);
    const selfieBtnColor = Color(0xFFD4AD6B);

    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        _buildReferencePunchCard(
          label: 'PUNCH IN',
          time: _formatTime(punchIn),
          icon: Icons.arrow_forward,
          cardColor: punchInBg,
          iconBgColor: punchInBg,
        ),
        const SizedBox(height: 12),
        _buildReferencePunchCard(
          label: 'PUNCH OUT',
          time: punchOut != null ? _formatTime(punchOut) : '----',
          icon: Icons.arrow_back,
          cardColor: punchOutBg,
          iconBgColor: punchOutIconBg,
        ),
          ...
          SizedBox(
            ...
            child: ElevatedButton.icon(
              ...
              style: ElevatedButton.styleFrom(
                backgroundColor: selfieBtnColor,
                foregroundColor: const Color(0xFF1A1E29),
  ...
  Widget _buildReferencePunchCard({
  ...
  Widget _buildHistoryList({int? limit}) {
    ...
    if (_activeFilter == 'All' ||
      ...
      // Month/Week history is derived from loaded month data
    } else {
      // filters like 'Late', 'Low Hours' work on the paginated _historyList
      displayList = _historyList.where((r) {
        if (_activeFilter == 'Late Check-in' || _activeFilter == 'Late') {
          return _isLateCheckIn(r['punchIn']);
