...
class _SalaryOverviewScreenState extends State<SalaryOverviewScreen> {
  ...
  Future<void> _fetchSalaryData({bool debounce = false}) async {
    ...
    try {
      ...
      // Present=1, Approved=1, Half Day=0.5 (weighted sum for salary)
      ...
      for (final record in _attendanceRecords) {
        final status = (record['status'] as String? ?? '').trim().toLowerCase();
        // Fine only for Present/Approved. Half Day must NOT incur late login fine.
        if (status == 'present' || status == 'approved') {
      ...
      // Pass only Present/Approved (utility excludes Half Day internally)
      if (dailySalary != null && dailySalary > 0) {
        final attendanceRecordsList = _attendanceRecords
            .where((record) {
              final s = (record['status'] as String? ?? '').trim().toLowerCase();
