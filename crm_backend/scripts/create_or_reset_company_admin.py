...
Usage:
  python scripts/create_or_reset_company_admin.py <company_email_or_name> <password>
...
def main():
    if len(sys.argv) < 3:
        print("Usage: python create_or_reset_company_admin.py <company_email_or_name> <password>")
        print("Example: python create_or_reset_company_admin.py \"suganthi2306m@gmail.com\" \"MyPass123\"")
        sys.exit(1)
    ...
    if not company_email:
        print("Company has no email set. Set company email first, then run this script with that email.")
        cur.close()
        conn.close()
        sys.exit(1)
    ...
    if existing:
        ...
        print(f"Password updated for company admin: {company_email}")
    else:
        cur.execute(
            """
            ...
            """,
            (company_name, company_email, hashed, company_id, rbac_role_id),
        )
        print(f"Company admin created for: {company_name} ({company_email})")
